...
//
// Rendered per request rather than cached: with a 30s revalidate window, the
// first visits after publishing were served the pre-publish page (stale while
// revalidating), so a new post looked missing. The lists come from the API
// container over the Docker network, so the extra render costs little.
...
async function getList(pathname) {
  try {
    const res = await fetch(`${apiBase()}${pathname}`, { cache: "no-store" });
